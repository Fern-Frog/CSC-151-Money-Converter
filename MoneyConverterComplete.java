...
public class MoneyConverterComplete extends JFrame {
    ...
    public MoneyConverterComplete(String name) {
        this.userName = name;
                    ...
                    if (savingEnabled) {
                        try {
                            ...
                            int open = JOptionPane.showConfirmDialog(
                                null,
                                "Open receipt file now?",
                                "Open File",
                                JOptionPane.YES_NO_OPTION
                            );
                            if (open == JOptionPane.YES_OPTION) {
                                Desktop.getDesktop().open(new File(userName + "Receipt.txt"));
                            }
    ...
    private void initComponents() {
        AnimatedGradientPanel gradientPanel = new AnimatedGradientPanel(new GridLayout(9, 1, 10, 10));
        gradientPanel.setBorder(BorderFactory.createEmptyBorder(10, 15, 10, 15));

        amountField = new JTextField();
        gradientPanel.add(new JLabel("Enter Amount:"));
        gradientPanel.add(amountField);

        fromCurrencyCombo = new JComboBox<>(currencyNames);
        gradientPanel.add(new JLabel("From Currency:"));
        gradientPanel.add(fromCurrencyCombo);

        toCurrencyCombo = new JComboBox<>(currencyNames);
        gradientPanel.add(new JLabel("To Currency:"));
        gradientPanel.add(toCurrencyCombo);
    ...
    private void convertCurrency() {
        ...
        result = Math.round(result * 100.0) / 100.0;  // round to 2 decimals

        String message = String.format("%.2f %s = %.2f %s", amount,
    ...
    private static class AnimatedGradientPanel extends JPanel implements ActionListener {
        ...
        protected void paintComponent(Graphics g) {
            ...
            Color c2 = Color.getHSBColor((hue + 0.5f) % 1f, 1f, 1f);
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            while (true) {
                ...
                name = name.trim();
                if (name.length() > 30) {
                    JOptionPane.showMessageDialog(null, "Name cannot exceed 30 characters. Please enter a shorter name.");
                    continue;
                }
            ...
            new MoneyConverterComplete(name);
        });
    }
}
