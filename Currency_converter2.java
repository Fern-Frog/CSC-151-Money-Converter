...
public class Currency_converter2 {

    // 39개의 개별 변수에 대한 임의의 숫자로 환율 설정
    ...
    static {
        // 39개의 옵션으로 OPTIONS 배열을 채우고 Option 1, Option 2 등으로 표시합니다.
        for (int i = 0; i < 39; i++) {
    ...
    public static void main(String[] args) {
        ...
        while (continueUsingApp) {
            ...
            // 39개의 옵션이 있는 드롭다운 메뉴(환율에 대한 자리 표시자로 숫자 사용)
            ...
            // 옵션 번호 추출(예: "Option 1" => 1, "Option 2" => 2, ...)
            ...
            // 사용자로부터 숫자 입력 받기
            String amountString = JOptionPane.showInputDialog(null, "Enter the amount to convert:", "Amount Input", JOptionPane.QUESTION_MESSAGE);
            double amount;
            try {
                ...
                continue; // 루프의 시작 부분으로 돌아가기
            }

            // 선택한 옵션 처리(현재 환율 표시만 해당)
            handleOption(selectedOption, selectedExchangeRate, amount);
    ...
    private static void handleOption(String selectedOption, double exchangeRate, double amount) {
        // 선택한 옵션과 관련 환율 표시
        ...
        panel.add(new JLabel("The converted amount is: " + convertedAmount));
